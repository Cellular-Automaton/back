defmodule Back.UserTest do
  use Back.DataCase

  alias Back.User

  describe "blocked" do
    alias Back.User.Blocked

    import Back.UserFixtures
    ...
    test "list_blocked/0 returns all blocked" do
      ...
      assert User.list_blocked() == [blocked]
    end

    test "get_blocked!/1 returns the blocked with given id" do
      blocked = blocked_fixture()
      assert User.get_blocked!(blocked.id) == blocked
    end
    ...
    test "create_blocked/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = User.create_blocked(@invalid_attrs)
    end
    ...
    test "update_blocked/2 with invalid data returns error changeset" do
      blocked = blocked_fixture()
      assert {:error, %Ecto.Changeset{}} = User.update_blocked(blocked, @invalid_attrs)
      assert blocked == User.get_blocked!(blocked.id)
    end

    test "delete_blocked/1 deletes the blocked" do
      blocked = blocked_fixture()
      assert {:ok, %Blocked{}} = User.delete_blocked(blocked)
      assert_raise Ecto.NoResultsError, fn -> User.get_blocked!(blocked.id) end
    end

    test "change_blocked/1 returns a blocked changeset" do
      blocked = blocked_fixture()
      assert %Ecto.Changeset{} = User.change_blocked(blocked)
    end
  end
end
